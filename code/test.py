...
def main(args, vis):
    ...
    val_loader = torch.utils.data.DataLoader(dataset,
                                             batch_size=int(args.batchSize), shuffle=False, num_workers=args.workers,
                                             pin_memory=True)
    ...
    with torch.no_grad():
        ...
        for b in range(0, imgs.shape[1], bsize):
            ...
            if b == 0:  # special case because dustbin has to be padded here
                dustbin_input = torch.cat([imgs[:, b:last_dim - 1], imgs[:, b + 1:last_dim]], dim=2)
            else:
                dustbin_input = torch.cat([imgs[:, b - 1:last_dim - 1], imgs[:, b:last_dim]], dim=2)
        ...
        for t in range(key_indices.shape[0]):
            ...
            if 'VIP' in args.filelist:
                outpath = os.path.join(args.save_path,
                                       'videos' + meta['img_paths'][t + n_context][0].split('videos')[-1])
...
def test(loader, model, args):
    ...
    for vid_idx, (imgs, imgs_orig, lbls, lbls_orig, lbl_map, meta) in enumerate(loader):
        test_fn(vid_idx, imgs, imgs_orig, lbls, lbls_orig, lbl_map, meta, model, args)
